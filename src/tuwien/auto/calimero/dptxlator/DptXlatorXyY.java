...
public class DptXlatorXyY extends DPTXlator {
	 ...
	 */
	public static final DPT DptColorXyY = new DPT("242.600", "color xyY", "(0, 0) 0", "(1, 1) 100", "%");

	private static final Map<String, DPT> types;

	static {
		types = new HashMap<>();
		types.put(DptColorXyY.getID(), DptColorXyY);
	}
	...
	private String fromDpt(final int index) {
		...
		if (values[0] != -1) {
			sb.append("(");
			sb.append(new DecimalFormat("#.####").format(values[0])).append(", ");
			sb.append(new DecimalFormat("#.####").format(values[1]));
			sb.append(")");
		}
		if (values[2] != -1) {
			if (sb.length() > 0)
	...
	protected void toDPT(final String value, final short[] dst, final int index) throws KNXFormatException {
		...
		final boolean clrValid = fields.length > 2 || fields[0].startsWith("(") || value.contains(", ");
		if (clrValid) {
			valid |= 2;
	...
	private short[] toDpt(final double x, final double y, final double brightness) {
		...
		return new short[] { valid, (short) (xRaw >> 8), ubyte(xRaw), (short) (yRaw >> 8), ubyte(yRaw),
			(short) Math.round(brightness * 255 / 100) };
	}
