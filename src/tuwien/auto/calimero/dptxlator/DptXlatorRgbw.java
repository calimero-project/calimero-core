...
public class DptXlatorRgbw extends DPTXlator {
	 ...
	 */
	public static final DPT DptColorRgbw = new DPT("251.600", "RGBW color", "0 0 0 0", "100 100 100 100", "%");

	private static final Map<String, DPT> types;

	static {
		types = new HashMap<>();
		types.put(DptColorRgbw.getID(), DptColorRgbw);
	}
	...
	private String component(final int index, final int component) {
		...
		final boolean valid = (data[index * 6 + 1] & mask) == mask;
		if (valid)
			return new DecimalFormat("##.#").format(data[index * 6 + 2 + component] * 100d / 255);
		return "-";
	}
	...
	private Optional<Double> component(final int component) {
		...
		final boolean valid = (data[index * 6 + 1] & mask) == mask;
		if (valid)
			return Optional.of(data[index * 6 + 2 + component] * 100d / 255);
	...
	protected void toDPT(final String value, final short[] dst, final int index) throws KNXFormatException {
		...
		for (int i = 0; i < 4; i++) {
			...
			if (!components[i].equals("-")) {
				valid |= mask;
				dst[index * 6 + 2 + i] = toDpt(components[i]);
			}
		}
		dst[index * 6 + 1] = (short) valid;
	}

	private short[] toDpt(final double red, final double green, final double blue, final double white) {
		final int valid = 0b1111;
		return new short[] { 0, valid, toDpt(red), toDpt(green), toDpt(blue), toDpt(white) };
	}
