 ...
 * <p>
 * State based datapoint interaction leads to transitions between states (so a datapoint value
 * associated to a certain datapoint represents a particular state over an amount of time). The
 * state of such a datapoint can be updated/invalidated by different (i.e., more than one) KNX group
 * addresses. An address marked as updating the state of this datapoint will allow KNX indication
 * and response messages with that destination address to set a new datapoint state value. An
 * address marked as invalidating the state of this datapoint will allow KNX indication messages
 * with that destination address to delete any state value information of the datapoint.<br>
 * This datapoint does not check for mutually exclusive containment of an address in one of those
 * both categories. The behavior when adding a KNX group address to both updating and invalidating
 * the datapoint state is undefined.<br>
 *
 * @author B. Malinowsky
 */
public class StateDP extends Datapoint
{
	...
	public StateDP(final GroupAddress main, final String name)
	{
		...
		invalidating = JavaME.synchronizedList();
		updating = JavaME.synchronizedList();
	}
	 ...
	 * @param main the group address used to identify this datapoint
	 * @param name user defined datapoint name
	 * @param invalidatingAddresses KNX group addresses, whose indication messages invalidate this
	 *        datapoint state
	 * @param updatingAddresses KNX group addresses, whose indication and response messages lead to
	 *        an update of this datapoint state
	 */
	public StateDP(final GroupAddress main, final String name,
	...
	{
		super(main, name, true);
		invalidating = JavaME.synchronizedList(invalidatingAddresses);
		updating = JavaME.synchronizedList(updatingAddresses);
	}
	 ...
	 * <p>
	 * This timeout specifies how long a local datapoint value is considered valid since it was
	 * received or got updated the last time in Calimero.<br>
	 * When working with state values related to this datapoint, this timeout should be queried
	 * using {@link #getExpirationTimeout()} to check whether there is a timeout set, and a value
	 * can still be considered valid / up to date. An expired state value is not intended to be used
	 * anymore, it should be discarded and requested/updated from the KNX network.<br>
	...
	/**
	 * Adds a group address to this datapoint to indicate that KNX messages with that address are
	 * allowed to alter the associated datapoint state (i.e., a state value related to this
	 * datapoint).
	 * <p>
	 * A group address can be marked as updating a state or invalidating a state. An address is
	 * added at most once to each category.
	 *
	 * @param a the KNX group address to add
	 * @param isUpdating <code>true</code> to mark the address as updating this datapoint state,
	 *        <code>false</code> to mark it as state invalidating
	 */
	...
	/**
	 * Returns the collection of KNX group addresses which are allowed to alter the state of this
	 * datapoint.
	 *
	 * @param updatingAddresses <code>true</code> if the updating addresses should be returned,
	 *        <code>false</code> for the invalidating addresses
	...
	{
		return JavaME.unmodifiableList(updatingAddresses ? updating : invalidating);
	}

	/**
	 * Returns whether KNX indication messages with destination group address <code>a</code> will
	 * invalidate the associated datapoint state of this datapoint.
	 *
	...
	/**
	 * Returns whether KNX indication or response messages with destination address <code>a</code>
