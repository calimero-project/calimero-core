...
public class XmlOutputFactory // extends XMLOutputFactory
{
	...
	private static final XmlOutputFactory f = new XmlOutputFactory();

	private final Map<String, Object> config = new HashMap<>();

	public static XmlOutputFactory getInstance()
	{
		return f;
	}
	...
	{
		if (!INTERNAL_ONLY) {
			System.out.println("lookup system-provided XMLStreamWriter");
			try {
				final XmlStreamWriterProxy w = XmlStreamWriterProxy.createXmlStreamWriter(stream);
				System.out.println("using StaX XMLStreamWriter " + w.w.getClass().getName());
				return w;
			}
			catch (Exception | Error e) {
				e.printStackTrace();
				throw new KNXMLException("createXMLStreamWriter", e);
				// fall-through to minimal writer implementation
			}
		}
//		final DefaultXmlWriter w = new DefaultXmlWriter(stream);
//		return w;
		throw new KNXMLException("no XML stream writer available");
	}
