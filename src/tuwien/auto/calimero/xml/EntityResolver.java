...
public final class EntityResolver
{
		...
		final String namespace) throws KNXMLException
	{
		try {
			try {
				return JavaME.newUrlInputStream(baseURI);
			}
			catch (final IOException e) {
				return JavaME.newFileInputStream(baseURI);
			}
		}
		catch (final IOException e) {
	...
	public OutputStream resolveOutput(final String systemID) throws KNXMLException
	{
		try {
			try {
				return JavaME.newUrlOutputStream(systemID);
			}
			catch (final IOException e) {
				return JavaME.newFileOutputStream(systemID);
			}
		}
		catch (final IOException e) {
	...
	public Reader getInputReader(final InputStream is) throws KNXMLException
	{
		try {
			...
			if (att[1] != null) {
				final String ianaEncoding = att[1].toUpperCase();
	...
	private static String getEncodingName(final InputStream is, final byte[] start, final int count)
	{
		...
		if (count == 4) {
			...
			final String[] encodings = { "ISO-10646-UCS-4", "ISO-10646-UCS-4", "UTF-16BE",
				"UTF-16LE", "CP037" };
	...
	private String[] readXMLDeclaration(final Reader r) throws KNXMLException
	{
		...
		try {
			...
			throw new KNXMLException("reading XML declaration, " + e.getMessage(), buf.toString(),
					0);
		}
			...
			if (state == 0 && s.startsWith("version")) {
				...
				if (s.startsWith("standalone")) {
					...
					if (!standalone.equals("yes") && !standalone.equals("no"))
						throw new KNXMLException("invalid standalone pseudo-attribute", standalone,
								0);
					break;
				}
			}
