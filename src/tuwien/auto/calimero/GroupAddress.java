 ...
 * <p>
 * Note, that the most significant bit of the main group (i.e., bit 15 in the unstructured address)
 * is reserved, but not in use for now. Hence this bit is not checked for, but nevertheless stored
 * and returned by this implementation.
 */
public class GroupAddress extends KNXAddress
{
	 ...
	 * A formatted address is a 2- or 3-level group address, the allowed separators are '.' or '/',
	 * mutually exclusive.
	 *
	 * @param address string containing the KNX address
	 * @throws KNXFormatException on unknown address type, wrong address syntax, group values out of
	 *         range, or wrong separator used
	 */
	 ...
	 * @throws KNXMLException if the xml element is no KNX address or the address couldn't be read
	 *         in correctly
	 */
	 ...
	 * <p>
	 * Note that, since this notation of levels only affects visual presentation and not internal
	 * operation, this is a class setting.
	 *
	 * @param format3Level <code>true</code> for 3-level group format, <code>false</code> for
	 *        2-level group format
	 */
	 ...
	 * <p>
	 * The main group is equal for both the 2- and 3-level group address notation (see class header
	 * specification).
	 *
	 ...
	 * <p>
	 * Depending on {@link #is3LevelPresentation()}, the address is formatted in 2- or 3-level group
	 * notation.
	 *
	 ...
	 * @return <code>true</code> iff <code>obj</code> is of this type and contains the same address
	 *         (raw), <code>false</code> otherwise
	 */
	@Override
