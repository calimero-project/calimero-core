 ...
 * <p>
 * A KNXnet/IP router is a fast replacement for line/backbone couplers and connected main/backbone
 * lines, using Ethernet cabling for example.<br>
 * The router use point to multipoint communication (multicast). By default, routers are joined to
 * the {@link KNXnetIPRouting#DEFAULT_MULTICAST} multicast group. On more KNX installations in one
 * IP network, different multicast addresses have to be assigned.<br>
 * All IP datagrams use UDP port number 3671, and only datagrams on this port are observed.<br>
 * The routing protocol is an unconfirmed service.
 * <p>
 * Optionally, a listener of type {@link RoutingListener} can be supplied to
 * {@link #addConnectionListener(KNXListener)} instead of a default {@link KNXListener}, to receive
 * {@link RoutingListener#lostMessage(LostMessageEvent)} notifications.
 * <p>
 * Multicast considerations:<br>
 * The multicast loopback behavior defines whether multicast datagrams are looped back to the local
 * socket, see {@link MulticastSocket#setLoopbackMode(boolean)}. By default, the loopback mode of
 * the multicast socket used for sending multicast datagrams is enabled. This behavior can be
 * changed by using a KNXnetIPRouting sub type and initializing it by calling
 * {@link #init(NetworkInterface, boolean, boolean)}.
 * <p>
 * A multicast datagram sent with an initial hop count greater 1 may be delivered to the sending
 * host on a different interface (than the sending one), if the host is a member of the multicast
 * group on that interface. The loopback mode setting of the sender's socket has no effect on this
 * behavior.
 *
 * @author B. Malinowsky
 */
public class KNXnetIPRouting extends ConnectionBase
{
	...
	private final EndpointAddress multicast;

	/**
	 * Creates a new KNXnet/IP routing service.
	 * <p>
	 * In general, routers are assigned a multicast address by adding an offset to the system setup
	 * multicast address ({@value #DEFAULT_MULTICAST}) for each KNX installation, by default this
	 * offset is 0 (i.e., only one used installation).
	 *
	 * @param netIf specifies the local network interface used to join the multicast group and send
	 *        outgoing multicast data, use <code>null</code> to use the default interface; useful
	 *        for multi-homed hosts
	 * @param mcGroup address of the multicast group this router is joined to, or <code>null</code>
	 *        to use the default multicast ({@value #DEFAULT_MULTICAST} ); value of
	 *        <code>mcGroup &ge; </code>{@value #DEFAULT_MULTICAST}
	 * @throws KNXException on socket error, or if joining to group failed
	 */
	public KNXnetIPRouting(final NetworkInterface netIf, final EndpointAddress mcGroup)
		throws KNXException
	{
		this(mcGroup);
	...
	/**
	 * Use this constructor in case initialization is called separately at a later point (using
	 * {@link #init(NetworkInterface, boolean, boolean)}).
	 * <p>
	 *
	 * @param mcGroup see {@link #KNXnetIPRouting(NetworkInterface, InetAddress)}
	 */
	protected KNXnetIPRouting(final EndpointAddress mcGroup)
	{
		super(KNXnetIPHeader.ROUTING_IND, 0, 1, 0);
		if (mcGroup == null)
			multicast = EndpointAddress.of(Discoverer.SEARCH_MULTICAST, DEFAULT_PORT);
		else if (!isValidRoutingMulticast(mcGroup))
			throw new KNXIllegalArgumentException("non-valid routing multicast " + mcGroup);
		else
			multicast = EndpointAddress.of(mcGroup.getHost(), DEFAULT_PORT);
	}
	 ...
	 * <p>
	 * This value is used to limit the multicast geographically, although this is just a rough
	 * estimation. The hop count value is forwarded to the underlying multicast socket used for
	 * communication.
	 *
	...
	public final void setHopCount(final int hopCount)
	{
		...
		try {
			((UDPMulticastConnection) socket).setTimeToLive(hopCount);
		}
	...
	{
		try {
			return ((UDPMulticastConnection) socket).getTimeToLive();
		}
		catch (final IOException e) {
	...
	/**
	 * Returns whether this KNXnet/IP routing instance has local loopback of KNXnet/IP routing
	 * multicast datagrams enabled or not.
	 *
	...
	{
		try {
			return !((UDPMulticastConnection) socket).isDisableLoopback();
		}
		catch (final IOException e) {
	 ...
	 */
	public static boolean isValidRoutingMulticast(final EndpointAddress address)
	{
		return address != null && address.isMulticast()
				&& toLong(address) >= toLong(Discoverer.SYSTEM_SETUP_MULTICAST);
	}

	/**
	 * Initialization routine to set the multicast network interface, loopback behavior and receiver
	 * loop.
	 * <p>
	 * Call this method only once during initialization.
	 *
	 * @param netIf see {@link #KNXnetIPRouting(NetworkInterface, InetAddress)}
	 * @param useMulticastLoopback <code>true</code> to loopback multicast packets to the local
	 *        socket, <code>false</code> otherwise; this parameter is only interpreted as a hint by
	 *        the operating system
	 * @param startReceiver <code>true</code> to start a threaded receiver loop which dispatches to
		...
		final boolean startReceiver) throws KNXException
	{
		...
		UDPMulticastConnection s = null;
		try {
			final AccessPoint[] ap = EndpointAddress.getByNetworkInterface(netIf);
			s = (UDPMulticastConnection) Connection.open(ap.length > 0 ? ap[0] : null, multicast);
			s.join(multicast.getHost());
		}
		catch (final IOException e) {
			if (s != null)
				try {
		...
		try {
			if (!useMulticastLoopback)
				s.setDisableLoopback(true);
			logger.info("multicast loopback mode "
					+ (s.isDisableLoopback() ? "disabled" : "enabled"));
		}
		catch (final IOException e) {
		...
		final int offset, final EndpointAddress sender) throws KNXFormatException, IOException
	{
		...
		if (h.getVersion() != KNXNETIP_VERSION_10)
			...
			return super.handleServiceType(h, data, offset, sender);
		return true;
	}
		...
		final Throwable t)
	{
		...
		try {
			((UDPMulticastConnection) socket).leave(multicast.getHost());
		}
		catch (final IOException e) {
	...
	private void fireLostMessage(final EndpointAddress sender, final RoutingLostMessage lost)
	{
		final LostMessageEvent e = new LostMessageEvent(this, sender, lost.getDeviceState(),
				lost.getLostMessages());
	...
	private static long toLong(final EndpointAddress addr)
	{
