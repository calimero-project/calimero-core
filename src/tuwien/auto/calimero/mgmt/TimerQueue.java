...
/**
 * A timer queue, which allows to submit timeouts and getting notified when that time span has
 * passed.
 * <p>
 * Its main purpose is to avoid a standard threaded destination timeout mechanism, which gets a
 * resource hog with respect to threads in KNX address scanning management procedures. It uses a
 * single thread approach to wait for the next submitted timeout to pass, and invokes the
 * corresponding notifiable for that timeout.<br>
 * The reason this class exists here is that we cannot depend on built-in functionality on J2ME
 * platforms deployed on resource-constrained devices.<br>
 * <p>
 * Implementation note: a newly submitted notifiable is always assumed to time out after any
 * previous submitted notifiable.
 *
 * @author B. Malinowsky
 */
			...
			while (true) {
				...
				synchronized (notifiables) {
					if (notifiables.isEmpty()) {
						...
						if (remaining <= 0 && !notifiables.isEmpty() && notifiables.get(0) == next
								&& endTimes.get(0) == end) {
