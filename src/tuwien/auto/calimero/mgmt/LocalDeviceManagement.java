...
import static tuwien.auto.calimero.cemi.CEMIDevMgmt.MC_PROPREAD_REQ;
...
abstract class LocalDeviceManagement<T> implements PropertyAdapter
{
	...
	private FuncPropResponse funcPropResponse(final CEMIDevMgmt req, final int serviceId)
			throws KNXRemoteException, InterruptedException {
		final var response = findFrame(CEMIDevMgmt.MC_FUNCPROP_CON, req);
	...
	{
		final var req = new CEMIDevMgmt(MC_PROPREAD_REQ, DEVICE_OBJECT, 1, PropertyAccess.PID.IO_LIST, 0, 1);
		send(req, BlockingMode.Confirmation);
		final int objects;
		try {
		...
		for (int startIndex = 1; startIndex <= objects; startIndex += stride) {
			...
			final var req2 = new CEMIDevMgmt(MC_PROPREAD_REQ, DEVICE_OBJECT, 1, PropertyAccess.PID.IO_LIST, startIndex, elements);
